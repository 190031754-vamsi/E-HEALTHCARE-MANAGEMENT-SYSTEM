...
<%
    ...
    if(session.getAttribute("Doctorusername") == null) {
        response.sendRedirect("DoctortLogin.jsp");
        return;
    }
%>
<nav>
    ...
    try {
        ...
        String query = "SELECT did, dname, qualification, specdoctor, email FROM doctors WHERE specdoctor = ?";
...
<div class="screen">
    ...
    <div class="slider">
        ...
        <!-- Section 2 -->
        <div class="header2">
        	<p>Doctor ID : <%=rs.getInt(1) %></p><br> 
        	<p>Doctor Name : <%=rs.getString(2) %></p><br> 
        	<p>Doctor Qualification : <%=rs.getString(3) %></p><br> 
        	<p>Doctor Name with Qualification : <%=rs.getString(4) %></p><br>
        	<p>Doctor Email : <%= rs.getString(5) %></p><br> 
        </div>
    </div>
</div>
<%
